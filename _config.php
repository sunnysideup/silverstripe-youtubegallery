...
//===================---------------- START youtubegallery MODULE ----------------===================
//YouTubeGalleryPage::set_youtube_video_url('http://www.youtube.com/watch?v=');
//YouTubeGalleryPage::set_player_width(425);
//YouTubeGalleryPage::set_player_height(344);
/*
YouTubeGalleryPage::set_player_options(
	array(
		"hl" => "en",
		"rel" => 0, 					//Values: 0 or 1. Default is 1. Sets whether the player should load related videos once playback of the initial video starts. Related videos are displayed in the "genie menu" when the menu button is pressed. The player search functionality will be disabled if rel is set to 0.
		"autoplay" => 0, 			//Values: 0 or 1. Default is 0. Sets whether or not the initial video will autoplay when the player loads.
		"loop" => 0,					//Values: 0 or 1. Default is 0. In the case of a single video player, a setting of 1 will cause the player to play the initial video again and again. In the case of a playlist player (or custom player), the player will play the entire playlist and then start again at the first video.
		...
		"start" =>  0, 				//Values: A positive integer. This parameter causes the player to begin playing the video at the given number of seconds from the start of the video. Note that similar to the seekTo function, the player will look for the closest keyframe to the time you specify. This means sometimes the play head may seek to just before the requested time, usually no more than ~2 seconds.
		"fs" => 0, 						//Values: 0 or 1. Default is 0. Setting to 1 enables the fullscreen button. This has no effect on the Chromeless Player. Note that you must include some extra arguments to your embed code for this to work. The bolded parts of the below example enable fullscreen functionality:
		"hd" => 0, 						//Values: 0 or 1. Default is 0. Setting to 1 enables HD playback by default. This has no effect on the Chromeless Player. This also has no effect if an HD version of the video is not available. If you enable this option, keep in mind that users with a slower connection may have an sub-optimal experience unless they turn off HD. You should ensure your player is large enough to display the video in its native resolution.
		"showsearch" => 0, 		//Values: 0 or 1. Default is 1. Setting to 0 disables the search box from displaying when the video is minimized. Note that if the rel parameter is set to 0 then the search box will also be disabled, regardless of the value of showsearch.
		"showinfo" => 0, 			//Values: 0 or 1. Default is 1. Setting to 0 causes the player to not display information like the video title and rating before the video starts playing.
		"iv_load_policy" => 0,//Values: 1 or 3. Default is 1. Setting to 1 will cause video annotations to be shown by default, whereas setting to 3 will cause video annotation to not be shown by default.
